...
echo "<script type=\"text/javascript\" src=\"/ko_admin/board/skin/".$board[skin]."/ajax.js\"></script>";
...
foreach($params AS $v){
	...
	$tmp_params = urldecode($v);
	$tom_ = explode("=", $tmp_params);
	${"$tom_[0]"} = $tom_[1];
//	echo $tom_[0] . " ======= ". ${"$tom_[0]"};
//	echo "<br />";
}
...
if($mode == "modify"){
	...
	if (isblank($board_id)){
		alert("비정상적 접근");
		echo "<script type=\"text/javascript\">do_Process('list', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '', '');</script>";
		exit;
	}
	if (isblank($name)){
		alert("이름을 입력해 주세요.");
	...
	if(!$_SESSION[is_admin]){
		if($board[is_membership] == "Y"){
			...
			if($_SESSION[member_id] != $check_row[id] ){
				alert("해당게시물은 관리자 및 작성자만 수정 가능합니다.");
				echo "<script type=\"text/javascript\">do_Process('list', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '', '');</script>";
				exit;
			}
	...
	alert("수정되었습니다.");
	echo "<script type=\"text/javascript\">do_Process('view', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '".$no."', '');</script>";
	exit;
	...
	if(!$_SESSION[is_admin]){
		if($board[is_membership] != "Y"){
			//비밀번호가 없음
			if($password == "" ){
				echo "<script>do_auth('auth', '$board_id', '$keyword', '$search_key', '$start', '$cate', '$no', '', 'delete');</script>";
				exit;
			}
			...
			if($check_row < 1 ){
				...
				echo "<script>do_Process('view', '$board_id', '$keyword', '$search_key', '$start', '$cate', '$no', '');</script>";
				exit;
			}
	...
	if($comment_mode != "delete"){
		...
		if (isblank($board_id)){
			alert("비정상적 접근");
			echo "<script type=\"text/javascript\">do_Process('view', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '".$ref_board_no."', '');</script>";
			exit;
		}
		if (isblank($comm_name)){
			alert("이름을 입력해 주세요.");
			echo "<script type=\"text/javascript\">do_Process('view', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '".$ref_board_no."', '');</script>";
			exit;
		}
	...
	if($comment_mode == "delete"){
		...
		if(!$_SESSION[is_admin]){
			if($board[is_membership] != "Y"){
				...
				if($check_row[password] != $comm_password){
					alert("비밀번호를 확인해주세요.");
					echo "<script type=\"text/javascript\">do_Process('view', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '".$ref_board_no."', '');</script>";
					exit;
				}
				...
				$check_row = mysqli_fetch_array(mysqli_query($dbp, "SELECT * FROM board_comment WHERE no = '$comm_no' LIMIT 1"));
				if($check_row[id] != $_SESSION[member_id]){
					alert("자신이 등록한 댓글만 삭제 할 수 있습니다.");
					echo "<script type=\"text/javascript\">do_Process('view', '".$board[id]."', '".$keyword."', '".$search_key."', '".$start."', '".$cate."', '".$ref_board_no."', '');</script>";
					exit;
				}

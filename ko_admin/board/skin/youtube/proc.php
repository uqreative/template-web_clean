...
if($mode == "modify"){
	...
	for($i = 1; $i <= $board[file_count]; $i++){
		...
		if( ${'del_files_'.$i} == "Y"){
			$file_check = mysqli_fetch_array(mysqli_query($dbp, "SELECT * FROM $board_id WHERE no='$no' LIMIT 1"));
			@unlink($dir.$file_check["file_".$i]);
			@unlink($dir."thumb_".$file_check["file_".$i]);

			//echo $dir.$file_check["file_".$i];
			//echo "<br />";
			//echo $dir."thumb_".$file_check["file_".$i];
			//exit;
			$add_query .= "file_".$i." =  '',";
		}

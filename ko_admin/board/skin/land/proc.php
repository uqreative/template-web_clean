...
foreach (array_keys($_SESSION) as $value) {
	if(is_array($_SESSION[$value])){
		${$value} = sanitizeString(join("|",$_SESSION[$value]));
	}else{
		${$value} = sanitizeString($_SESSION[$value]);
	}
	// ${$value} = sanitizeMySQL($_POST[$value]);
}
foreach (array_keys($_POST) as $value) {
	if(is_array($_POST[$value])){
		${$value} = sanitizeString(join("|",$_POST[$value]));
	}else{
	...
	for($i = 1; $i <= $board[file_count]; $i++){
		if($_FILES["file_".$i][size] > 0){
			if ($_FILES["file_".$i][size] > $board[file_limit_size] * 1024 * 1024) {
				...
				$file_query_arr[] = "file_".$i." =  '". ${'file_'.$i}."'";
			}
		}
	}

	if($_FILES["certificate"][size] > 0){
		if ($_FILES["certificate"][size] > $board[file_limit_size] * 1024 * 1024) {
			error("첨부파일 ".$i."의 용량 제한은 $board[file_limit_size]M 입니다.");
			exit;
		} else {
			$certificate = upload_file($dir, $_FILES["certificate"][tmp_name], $_FILES["certificate"][name]);
			$file_query_arr[] .= "certificate =  '". $certificate."'";
		}
	}
	if(count($file_query_arr) > 0)
		$file_query = " , ".join("," , $file_query_arr);

	$escape_field = array('id','file_1','file_2','file_3','file_4','file_5','file_6','file_7','file_8','file_9','file_10','certificate');
	$add_query = make_set_query_str($board_id,$escape_field);

	$update_query = "UPDATE $board_id SET id='$_SESSION[member_id]',{$add_query} {$file_query} WHERE no='$no' LIMIT 1";
...
$comm_comments = $_GET[comm_comments];
	...
	$insert_query = "INSERT $board_id SET id='$_SESSION[member_id]',
											CI='$_SESSION[CI]',
											DI='$_SESSION[DI]',
											name='$name',
											password='$password',
	...
	$escape_field = array('id');
	$add_query = make_set_query_str($board_id,$escape_field);

	$insert_query = "INSERT $board_id SET id='$_SESSION[member_id]',{$add_query}";

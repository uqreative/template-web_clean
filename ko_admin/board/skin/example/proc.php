...
if($mode == "modify"){
	...
	for($i = 1; $i <= $board[file_count]; $i++){
		if(${'del_'.$i} == "Y" && $_FILES["file_".$i][size] > 0){
			@unlink($dir . $modify_row['file_'.$i]);
			@unlink($dir . "thumb_".$modify_row['file_'.$i]);
	...
	$update_query = "UPDATE $board_id SET name='$name',
											password='$password',
											phone='$phone',
											email='$email',
											zip='$zip',
											...
											file_type='$file_type',
											$add_query
											useDownload_1='$useDownload_1',
											useDownload_2='$useDownload_2',
											useDownload_3='$useDownload_3',
											useDownload_4='$useDownload_4',
											useDownload_5='$useDownload_5',
											useDownload_6='$useDownload_6',
											useDownload_7='$useDownload_7',
											useDownload_8='$useDownload_8',
											useDownload_9='$useDownload_9',
											useDownload_10='$useDownload_10',
											ip='$ip',
	...
	//파일 있는지 체크
	for($i = 1; $i <= 10; $i++){
		if($modify_row{'file_'.$i}){
			$update_add_query .= "useDownload_".$i." =  '". ${'useDownload_'.$i} . "',";
		} else {
			$update_add_query .= "useDownload_".$i." =  'N',";
		}
	}

	$update_query = "UPDATE $board_id SET $update_add_query
										hidden='$hidden'
									WHERE no='$no' LIMIT 1";
...
$comm_comments = $_GET[comm_comments];
	...
	$check_ = mysqli_fetch_array(mysqli_query($dbp, "SELECT * FROM $board_id WHERE ref_no='$ref_no' AND depth > 0 ORDER BY ref_group DESC LIMIT 1"));
	$ref_group = $check_['ref_group'];
	if($check_[ref_group] != 0){
		$ref_group = $check_[ref_group] + 1;
	} else {
		$ref_group = 1;
	}
	...
	//mysql_query("INSERT INTO $board_id VALUES ('', '$depth', '$ref_no', '$ref_group', '$_SESSION[member_id]', '$_SESSION[CI]', '$_SESSION[DI]', '$name', '$password', '$phone','$email', '$zip', '$address1', '$address2', '$category', '$title', '$comments_type', '$tag_type', '$comments', '$etc', '$notice', '$secret', '$file_type', '$file_1', '$file_2', '$file_3', '$file_4', '$file_5', '$file_6', '$file_7', '$file_8', '$file_9', '$file_10', '$reg_date', '$ip', '$reply_state', '$reply_id', '$reply_name', '$reply_phone', '$reply_email', '$reply_comments', '$reply_ip', '$reply_date', '$reply_file_1', '$reply_file_2', '$reply_file_3', '$reply_file_4', '$reply_file_5', '$view_count', '$hidden', '$link')");
	...
	mysqli_query($dbp, "UPDATE $board_id SET ref_no = '$ref_no' WHERE no='$ref_no' LIMIT 1");

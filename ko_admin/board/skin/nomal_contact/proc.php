...
$board = mysqli_fetch_array(mysqli_query($dbp, "SELECT * FROM board_config WHERE id='$board_id' LIMIT 1"));
$view_count = 0;
$ip = $REMOTE_ADDR;
...
$comm_comments = $_GET[comm_comments];
	...
	if($comment_mode == "delete"){
		...
		$check_row = mysqli_fetch_array(mysql_query($dbp, "SELECT * FROM board_comment WHERE ref_group = '$ref_group' ORDER BY ref_depth DESC LIMIT 1"));
	...
	alert("등록되었습니다.");
	sms_send($site[sms_id], $site[sms_key], "010-2508-5133", "010-5444-8612", "[".$site[title]."] 견적문의 $comments_type $phone");
	url($return_url[0]."?board_id=$board_id");

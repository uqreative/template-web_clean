<?php
	...
	if (($inTpBitVal & 1) == 1) {
		$name = $_POST["name"];				// 성명
		if (preg_match('~[^\xA1-\xFEa-zA-Z ]~', $name, $match)) {	// EUC-KR인 경우
	...
	// ########################################################################
	//$memId = "V12960000000";										// 회원사코드(아이디)
	$memId = $site[namecheck_key];										// 회원사코드(아이디)

	// ########################################################################
	// # 회원사 모듈설치서버 IP 및 회원사 도메인 설정
	// ########################################################################
	$serverIp = "x";					// 모듈이 설치된 서버IP (서버IP검증을 무시하려면 'x'로 설정)
	$siteDomain = "ok-name.co.kr";		// 회원사 도메인. (휴대폰인증번호 발송시 제휴사명에 노출)
	...
	$returnUrl = "http://".$_SERVER['HTTP_HOST']."/okname/auth_proc.php";// 본인인증 완료후 리턴될 URL (도메인 포함 full path)
